...
class CrosswordCreator():
    ...
    def revise(self, x, y):
        ...
        if len(conflict) > 0:
            result = True
            self.domains[x] = self.domains[x] - conflict
        return result
    ...
    def consistent(self, assignment):
        ...
        if len(values) != len(set(values)):
            return False
        for key, v in self.crossword.overlaps.items():
            if v is None:
                continue
            x, y = key
            i, j = v
    ...
    def backtrack(self, assignment):
        ...
        dv = self.order_domain_values(var, assignment)
        for word in dv:
            assignment[var] = word
